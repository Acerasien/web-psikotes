...
function ParticipantsPage({ token }) {
    ...
    useEffect(() => {
        ...
        const fetchAssignments = async () => {
            try {
                const r = await axios.get('http://127.0.0.1:8000/assignments/', { headers: { Authorization: `Bearer ${token}` } });
    ...
    const handleAssign = async (userId) => {
        ...
        if (!testId) { Swal.fire("Error", "Select a test first", "error"); return; }
        try {
            await axios.post(`http://127.0.0.1:8000/assignments/?user_id=${userId}&test_id=${testId}`, {}, { headers: { Authorization: `Bearer ${token}` } });
            fetchAssignments();
        } catch (err) { Swal.fire("Error", err.response?.data?.detail || "Unknown", "error"); }
    };
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white shadow overflow-hidden sm:rounded-lg">
                <div className="px-4 py-5 sm:px-6 border-b border-gray-200 flex justify-between items-center">
                    ...
                    <input type="text" placeholder="Search..." className="border rounded px-3 py-1 text-sm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                </div>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <div className="flex flex-col gap-1">
                                            {getUserAssignments(u.id).length > 0 ? getUserAssignments(u.id).map(a => (
                                                <span key={a.id} className={`px-2 py-1 rounded text-xs inline-block ${a.status === 'locked' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
                                        ...
                                        <div className="flex gap-2 items-center">
                                            {getUserAssignments(u.id).some(a => a.status === 'locked') && (
                                                <button onClick={async () => {
                                                    ...
                                                    Swal.fire({ title: 'Unlock?', icon: 'warning', showCancelButton: true }).then(async (res) => {
                                                        if (res.isConfirmed) {
                                                            await axios.post(`http://127.0.0.1:8000/admin/assignments/${lockedA.id}/unlock`, {}, { headers: { Authorization: `Bearer ${token}` } });
                                                            fetchAssignments();
                                                            Swal.fire('Unlocked!', '', 'success');
                                                        }
                                                    });
                                                }} className="bg-red-500 text-white text-xs px-2 py-1 rounded">Unlock</button>
                                            )}
                                            <select className="border rounded px-2 py-1 text-xs" onChange={(e) => setSelectedTest({ ...selectedTest, [u.id]: e.target.value })} defaultValue="">
                                                <option value="" disabled>Select</option>
                                                {tests.filter(t => !isTestAssigned(u.id, t.id)).map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                                            </select>
                                            <button onClick={() => handleAssign(u.id)} className="bg-blue-500 text-white text-xs px-2 py-1 rounded">Add</button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                ...
                <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
                    <div className="text-sm text-gray-700">Showing {indexOfFirstUser + 1} to {Math.min(indexOfLastUser, filteredUsers.length)} of {filteredUsers.length}</div>
                    <div className="flex gap-2">
                        ...
                        <button onClick={() => setCurrentPage(prev => prev + 1)} disabled={indexOfLastUser >= filteredUsers.length} className="px-3 py-1 border rounded">Next</button>
                    </div>
                </div>
            </div>
        </div>
